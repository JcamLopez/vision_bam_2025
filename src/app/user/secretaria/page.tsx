...
import { obtenerSolicitudes } from "@/services/frontend/solicitudesServices";
...
export default function Solicitudes() {
  ...
  const permisos = ['PERMISO 1', 'PERMISO 2'];
  ...
  const confirmarAccion = async () => {
    ...
    console.log(`${accionPendiente === 'aceptar' ? '✅ Aceptada' : '❌ Rechazada'}:`, solicitudSeleccionada);
    cerrarModal();
  };
  ...
  useEffect(() => {
    ...
    if (usuarioGuardado) {
      ...
      const permisosAdaptados: Permiso[] = (user.permisos || []).map((p: any) =>
        typeof p === 'string'
          ? { nombre: p, ruta: '#' } // valor por defecto si no hay ruta
